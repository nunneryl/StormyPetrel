...
# Durable, COMMITTED id->lat/lng snapshot of NDBC buoys. The activestations.xml above is downloaded
# and ephemeral; this snapshot lets buoy assignments be validated at import time and audited in SQL
# without a live NDBC fetch. Regenerated by `python -m pipeline.snapshot_buoys` (needs the XML present)
# and mirrored into the `buoys` table by db_import.
NDBC_BUOY_SNAPSHOT_FILE = PIPELINE_DIR / "data" / "ndbc_buoy_snapshot.json"
# Import-time consistency guard: the stored nearest_buoy_dist_km / nearest_tide_station_dist_km must
# agree with the great-circle distance to the assigned station's snapshot coords within this tolerance
# (or the station's regional cap). A grosser mismatch is a stale/corrupt pairing and is flagged.
COORD_DERIVED_DIST_TOLERANCE_KM = 5.0
